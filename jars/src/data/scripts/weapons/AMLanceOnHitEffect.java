...
public class AMLanceOnHitEffect implements OnHitEffectPlugin {
    ...
    private static final String SOUND_ID = "brdy_amlance_impact";
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ...
        if (target instanceof ShipAPI && !shieldHit && Math.random() <= EXTRA_DAMAGE_CHANCE) {
            ...
            loc.x -= FastTrig.cos(target.getFacing() * Math.PI / 180f);
            loc.y -= FastTrig.sin(target.getFacing() * Math.PI / 180f);
            ShipAPI ship = (ShipAPI) target;
            AnamorphicFlare.createFlare(ship, new Vector2f(loc), engine, 1f, 0.05f, 0f, 10f, 1f, PARTICLE_COLOR, EXPLOSION_COLOR);

            Vector2f vel = new Vector2f(target.getVelocity());
            vel.scale(0.45f);
            engine.spawnExplosion(point, vel, EXPLOSION_COLOR, 295f, 3.5f);
            float speed = projectile.getVelocity().length();
            float facing = projectile.getFacing();
            for (int x = 0; x < NUM_PARTICLES; x++) {
                engine.addHitParticle(point, MathUtils.getPointOnCircumference(null, MathUtils.getRandomNumberInRange(
                        speed * .0006f, speed * .044f),

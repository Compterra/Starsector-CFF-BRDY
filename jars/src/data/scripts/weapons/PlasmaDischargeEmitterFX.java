...
public class PlasmaDischargeEmitterFX implements EveryFrameWeaponEffectPlugin {

    private static final float FIRING_DURATION = 1.45f; // weapon_data.csv: (chargeup + chargedown)
    private static final Color MUZZLE_FLASH_COLOR = new Color(119, 200, 137, 245);
    private static final float OFFSET = 11f;
    private static final Color PARTICLE_COLOR = new Color(119, 200, 137, 235);
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (weapon.isFiring()) {
            ...
            if (elapsed <= 0f) {
                ...
                engine.spawnExplosion(explosion_offset, ship.getVelocity(), MUZZLE_FLASH_COLOR, 75f, 0.35f);
            }
            ...
            for (int x = 0; x < particle_count_this_frame; x++) {
                ...
                angle = weapon.getCurrAngle() + TrylobotUtils.get_random(-14f, 14f);
                velocity = TrylobotUtils.translate_polar(ship.getVelocity(), speed, angle);
                engine.addHitParticle(particle_offset, velocity, size, 1.3f, 0.25f, PARTICLE_COLOR);
            }
        } else {
            elapsed = 0f;
        }

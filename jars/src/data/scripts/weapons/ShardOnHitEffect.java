...
public class ShardOnHitEffect implements OnHitEffectPlugin {
    ...
    private static final float EXTRA_DAMAGE_CHANCE = 0.14f;
    private static final float MAX_EXTRA_DAMAGE = 200f;
    private static final float MIN_EXTRA_DAMAGE = 150f;
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ...
        if (target instanceof ShipAPI && !shieldHit && Math.random() <= EXTRA_DAMAGE_CHANCE) {
            ...
            vel.scale(0.45f);
            engine.spawnExplosion(point, vel, EXPLOSION_COLOR, 46f, 1.5f);

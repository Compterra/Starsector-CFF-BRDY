...
public class IWShardOnHitEffect implements OnHitEffectPlugin {
    ...
    private static final float EXTRA_DAMAGE_CHANCE = 0.35f;
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ...
        if (target instanceof ShipAPI && !shieldHit && Math.random() <= EXTRA_DAMAGE_CHANCE) {
            ...
            engine.applyDamage(target, point, MathUtils.getRandomNumberInRange(MIN_EXTRA_DAMAGE, MAX_EXTRA_DAMAGE), TYPES[(int) ((float) Math.random()
            ...
            vel.scale(0.48f);
            engine.spawnExplosion(point, vel, EXPLOSION_COLOR, 39f, 3f);
            float speed = projectile.getVelocity().length();
            float facing = projectile.getFacing();
            for (int x = 0; x < NUM_PARTICLES; x++) {
                engine.addHitParticle(point, MathUtils.getPointOnCircumference(null, MathUtils.getRandomNumberInRange(speed * .007f, speed * .17f),
                                                                               MathUtils.getRandomNumberInRange(facing - 180f, facing + 180f)), 5f, 1f, 1.6f,
                                      PARTICLE_COLOR);
            }

...
public class MicroShardOnHitEffect implements OnHitEffectPlugin {
    ...
    private static final float EXTRA_DAMAGE_CHANCE = 0.20f;
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ...
        if (target instanceof ShipAPI && !shieldHit && Math.random() <= EXTRA_DAMAGE_CHANCE) {
            ...
            DamageType.ENERGY, 0f, false,
                               true, projectile.getSource());

            // Spawn visual effects
            Vector2f vel = new Vector2f(target.getVelocity());
            vel.scale(0.48f);
            engine.spawnExplosion(point, vel, EXPLOSION_COLOR, 39f, 1f);

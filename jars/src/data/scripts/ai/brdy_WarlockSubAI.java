...
import data.scripts.weapons.MageOnHit;
...
public class brdy_WarlockSubAI implements MissileAIPlugin, GuidedMissileAI {

    private static final float LOW_ACCURACY_FACTOR = 1.25f;
    private static final float WEAVE_TIME_FACTOR = 9f;
    ...
    public brdy_WarlockSubAI(MissileAPI missile, ShipAPI launchingShip) {
    ...
    public void advance(float amount) {
        if (missile.isFizzling() || missile.isFading()) {
            if (Math.random() > 0.95) {
                MageOnHit.pop(missile.getLocation(), Global.getCombatEngine());

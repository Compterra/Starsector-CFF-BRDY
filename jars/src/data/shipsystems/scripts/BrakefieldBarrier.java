...
public class BrakefieldBarrier extends BaseShipSystemScript {

    private static final float FRIGATE_MOD = 0.1f;
    private static final float DESTROYER_MOD = 0.2f;
    private static final float CRUISER_MOD = 0.3f;
    private static final float CAPITAL_SHIP_MOD = 0.4f;

    private static final Color FIELD_COLOR = new Color(45, 148, 34, 19); // some color idk
    private static final float FIELD_RANGE = 250f;
    private static final float VISUAL_RANGE = 275f;    
    private static final float FIELD_STRENGTH = 10f;
    private static final float MAXIMUM_MASS = 2000f;
    private static final float MAX_FRACTION = 3f;
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        while (accum >= (1f / 60f)) {
            ...
            Global.getCombatEngine().addSmoothParticle(ship.getLocation(), ZERO, VISUAL_RANGE, 1f * effectLevel, 0.2f * effectLevel, FIELD_COLOR);
    ...
    public StatusData getStatusData(int index, State state, float effectLevel) {
        if (index == 0) {
            return new StatusData("stopping enemy projectiles", false);
        }
        return null;
    }
